...
def round_filters(filters, global_params):
    """Calculate and round number of filters based on width multiplier. Use
    width_coefficient, depth_divisor and min_depth of global_params.
...
def calculate_output_image_size(input_image_size, stride):
    """Calculates the output image size when using Conv2dSamePadding with a
    stride. Necessary for static padding. Thanks to mannatsingh for pointing
    this out.
...
def get_same_padding_conv2d(image_size=None):
    ...
    padding otherwise. Static padding is necessary for ONNX exporting of
    models.
...
class Conv2dDynamicSamePadding(nn.Conv2d):
    ...
    The padding is operated in forward function by calculating
    dynamically.
    """
...
class Conv2dStaticSamePadding(nn.Conv2d):
    """2D Convolutions like TensorFlow's 'SAME' mode, with the given input
    image size.

    The padding module is calculated in construction function, then used
    in forward.
    """
...
def get_model_params(model_name, override_params):
    ...
    if override_params:
        # ValueError will be raised here if override_params has fields not included in global_params.  # NOQA
